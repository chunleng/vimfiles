...
function f_assignment(args)
    ...
    for i = 1, #arglist do
        arglist[i] = arglist[i]:gsub('^%s*([a-zA-Z_]+[a-zA-Z_0-9]*).*$',
                                     'this.%1 = %1;')
    end
    return isn(nil, t(arglist), '$PARENT_INDENT\t\t')
end
table.insert(M, s({trig = 'c', dscr = 'Class'}, c(1, {
    fmta([[
		class <> {
			constructor(<>) {
				<>
	...
	]], {r(1, 'name'), r(2, 'args'), d(3, f_assignment, {2})}), fmta([[
		const <> = class {
			constructor(<>) {
				<>
			}
...
table.insert(M, s({trig = 'lw', dscr = 'Loop (while)'}, c(1, {
    fmta([[
		while (<>) {
...
table.insert(M, s({trig = 'err', dscr = 'Raise Exception'}, fmta([[
	throw new <>(<>);
]], {i(1, 'Error'), i(2)})))
