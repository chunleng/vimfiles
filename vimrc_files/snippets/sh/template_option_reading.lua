...
table.insert(
	...
	test "$1" = "--" && shift
			...
			{
				i(0, "[Description of application here]"),
				d(2, function(args)
					local flags = get_flags(args[1][1])

					for i = 1, #flags do
						flags[i] = string.format("-%-9s  [Description of this option]", flags[i]:gsub(":", " <value>"))
					end
				...
				d(3, function(args)
					...
					for i = 1, #flags do
						...
						if string.find(flags[i], ":") then
							table.insert(lines, string.format('\techo "%s ${OPTARG}"', flags[i]:sub(1, 1)))
						else
							table.insert(lines, string.format('\techo "%s"', flags[i]:sub(1, 1)))
						end
						table.insert(lines, "\t;;")
					end
