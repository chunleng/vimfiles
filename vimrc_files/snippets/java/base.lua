...
table.insert(M, s({trig = 'imp', dscr = 'Import'},
                  fmta('import <>;', {i(0, 'lib')})))
...
table.insert(M, s({trig = 'if', dscr = 'Interface'}, fmta([[
	public interface <> {
		<>
	}
...
table.insert(M, s({trig = 'l', dscr = 'Loop (for)'}, fmta([[
    ...
    c(1, {
        fmta('<> : <>',
             {i(1, 'Map.Entry<String, String> arr'), i(2, 'hm.entrySet()')}),
        fmt('int {} = 0; {} < {}; {}++',
            {i(1, 'i'), l(l._1, 1), i(2, '10'), l(l._1, 1)})
    }), i(0)
})))
...
table.insert(M, s({trig = 'T', dscr = 'False'}, fmta([[
	true
]], {})))
