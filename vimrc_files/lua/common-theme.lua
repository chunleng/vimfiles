...
local function update_color(colors)
	for i, value in ipairs(colors) do
		M.colormap[i - 1] = value
	end
end

local function update_blender(blenders)
	for key, value in pairs(blenders) do
		M.blender[key] = value
	end
end
...
function M.set_hl(name, options)
	...
	if options.sp ~= nil then
		options.sp = M.colormap[options.sp]
	end
...
function M.setup()
	...
	M.set_hl("DiagnosticUnderlineError", { undercurl = true, sp = M.blender.error })
	...
	M.set_hl("DiagnosticVirtualText", { undercurl = true, bold = true, fg = M.blender.fg_darker_3 })
	...
	M.set_hl("StatusLine", { fg = M.blender.fg_darker_1, bg = M.blender.bg_lighter_1 })
