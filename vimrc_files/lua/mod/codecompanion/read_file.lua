-- Reference to https://github.com/olimorris/codecompanion.nvim/blob/42cf6d1637fc8934421fe69745fd01098068ece6/lua/codecompanion/interactions/chat/tools/builtin/read_file.lua
-- Added a few features so that we can adjust the options
-- - validate_cwd: Return error when File is outside of current working directory
...
local function read(action, opts)
	local path = helpers.validate_and_normalize_path(action.filepath)
	local p = Path:new(path)
	if opts.validate_cwd then
		local pathh = vim.fn.fnamemodify(action.filepath, ":p")
		local cwd = vim.fn.getcwd() .. "/"
		if pathh:sub(1, #cwd) ~= cwd then
			return {
				status = "error",
...
return {
	...
	cmds = {
		...
		function(self, args, input)
			return read(args, self.tool.opts)
		end,
	},
	schema = {
		type = "function",
	...
	output = {
		...
		success = function(self, tools, cmd, stdout)
			...
			local range_text = end_line == -1 and fmt("(%d - end)", start_line)
				or fmt("(%d - %d)", start_line, end_line)
