-- Duplicate
-- https://github.com/olimorris/codecompanion.nvim/blob/d5b129c65b243a368871dc3f2705642467995c0d/lua/codecompanion/interactions/chat/tools/builtin/insert_edit_into_file/init.lua
-- because buffer is not saved if require_confirmation_after is set to false
--[[
...
local function handle_user_decision(opts)
	...
	return wait.for_decision(
		opts.diff_id,
		{ "CodeCompanionDiffAccepted", "CodeCompanionDiffRejected" },
		function(result)
			local response
					...
					if is_buffer then
						...
						response = error_response(
							result.timeout and "User failed to accept the edits in time"
								or fmt(
									'User rejected the edits for `%s`, with the reason "%s"',
									opts.display_name,
									reason
								)
						)
